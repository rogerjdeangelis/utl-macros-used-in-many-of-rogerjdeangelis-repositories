...
%macro prtwha;
   ...
   title1 "print of datasets &argd";
   title2 &argx;
   proc print data=&argd(obs=500 where=(%sysfunc(dequote(&argx))))  width=min;
   run;
   title;
   dm "out;top;";
%mend prtwha;
...
%local uj revslash _wrkstm _subdir _dir;
...
/*
C:\Users\rdeangelis\AppData\Local\Temp\SAS Temporary Files\_TD19424_RDEANGELISL5410_
*/

data _null_;
   rc=filename('__xplp', "&_dir./__xplp");
   if rc = 0 and fexist('__xplo') then rc=fdelete('__xplp');
   rc=filename('__xplp');
run;

%let revslash=%sysfunc(translate(&_dir.,'/','\'));
%put &=revslash;
run;quit;
...
%macro utl_submit_py64_38(
  ...
  * write the program to a temporary file;
  ...
  filename py_pgm "&_dir./py_pgm.py" lrecl=32766 recfm=v;
  data _null_;
...
%macro debuga;
   ...
   %let _dir=&_wrkstm./&_subdir;
   %put &_dir;
   %let rc=%sysfunc(filename(myRef,&_dir./mactxt.sas));
   %let sysrc=%sysfunc(fdelete(&myRef));
   %let rc=%sysfunc(filename(&myref));
   filename clp clipbrd ;
   data _null_;
     ...
     put _infile_;
   run;
   filename mprint  "&_dir./mactxt.sas";
   options mfile mprint source2;
   %inc "&_dir./macraw.sas";
   run;quit;
   options nomfile nomprint;
